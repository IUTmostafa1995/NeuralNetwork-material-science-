...
cm = CoulombMatrix(
    n_atoms_max=45,  permutation='sorted_l2',flatten=True)
# Setup
mbtr = MBTR(
    species=["Si"],
    k1={
        "geometry": {"function": "atomic_number"},
        "grid": {"min": 10, "max": 18, "n": 50, "sigma": 0.1},
    },
    k2={
...
for i in range(len(list_files)):
      src="/home/mostafa/BAND_STRUCTURES/Si-semiconductors/QE_files/Si"+a[i][0]+"/s"+a[i][1]
      atom=read_vasp(src+"/POSCAR_relaxed")
      mbtr_atom = mbtr.create(atom)
      #mbtr_atom = soap.create(atom)
      #mbtr_atom = cm.create(atom)
      mbtr_features.append(mbtr_atom)  
...
feature=feature.to_numpy('float64')
feature=np.hstack((feature,mbtr_features))
...
rf_reg = RandomForestRegressor(n_estimators=100,criterion=criterion)#, random_state=42)
...
pred_train = rf_reg.predict(X_train_scaled)
#pred_spec= rf_reg.predict(feature2)
...
##explainer = shap.Explainer(rf_reg.predict, X_train_scaled)
